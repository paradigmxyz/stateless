...
macro_rules! fixtures_test {
    ($test_name:ident, $dir:expr) => {
        #[test]
        fn $test_name() {
            reth_tracing::init_test_tracing();
            // Ensure this path matches where your fixtures are actually stored
            let suite_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("execution-spec-tests");

            BlockchainTests::new(suite_path).run_only($dir);
        }
    };
}

fixtures_test!(test_amsterdam, "for_amsterdam");
